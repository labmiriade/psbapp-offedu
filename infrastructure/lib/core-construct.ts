...
export interface CoreConstructProps {
  ...
  /**
   * The educational offers csv data urls
   */
  csvDataUrls: string;
  /**
...
export class CoreConstruct extends Construct {
  constructor(scope: Construct, id: string, props: CoreConstructProps) {
    ...
    // lambda to import educational offers to dynamodb
    const offEduImportLambda = new lambda.Function(this, 'OffEduImportFn', {
      code: new lambda.AssetCode('../etl/import-offedu', {
        bundling: {
      ...
      memorySize: 512,
      description: 'Lambda per importare offerte formative',
      environment: {
        DATA_TABLE: dataTable.tableName,
        CSV_DATA_URLS: props.csvDataUrls,
      },
    });
    dataTable.grantReadWriteData(offEduImportLambda);

    const offEduImportErrors = offEduImportLambda.metricErrors({
      period: cdk.Duration.minutes(1),
    });

    const importAlarm = new cloudwatch.Alarm(this, 'offEduImportErrorsAlarm', {
      metric: offEduImportErrors,
      threshold: 1,
      comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
      evaluationPeriods: 1,
      alarmDescription: 'An error occurred during the OffEduImport function execution',
    });
    ...
    eventRule.addTarget(new targets.LambdaFunction(offEduImportLambda));
